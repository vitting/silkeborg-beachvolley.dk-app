...
class AdminWriteToSentMessages extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return StreamBuilder(
        stream: WriteToData.getAllMessagesSentMessage(),
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          if (!snapshot.hasData) return LoaderSpinner();
          if (snapshot.hasData && snapshot.data.documents.length == 0)
            return NoData(FlutterI18n.translate(
                context, "writeTo.adminWriteToSentMessages.string1"));
  ...
  Future<bool> _deleteMessage(BuildContext context) async {
    ...
    int result = await Dialogs.modalBottomSheet(context, [
      DialogsModalBottomSheetItem(
          FlutterI18n.translate(context, "writeTo.adminWriteToSentMessages.string2"),
          Icons.delete,
          0)
    ]);
    ...
    if (result != null && result == 0) {
      ConfirmDialogAction action = await Dialogs.confirmDelete(context,
          FlutterI18n.translate(context, "writeTo.adminWriteToSentMessages.string3"));
