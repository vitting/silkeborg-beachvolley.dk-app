  ...
  if (Home.loggedInUser != null) {
    ...
    if (enrollmentExists.emailExists || enrollmentExists.phoneExists) {
      ConfirmDialogAction result = await confirmDialogFunctions.confirmDialog(
          context,
          title: Text(FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string1")),
          actionLeft: ConfirmDialogAction.no,
          actionRight: ConfirmDialogAction.yes,
          body: <Widget>[
            _getDialogText(context, enrollmentExists),
            Text(FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string2"))
          ]);
...
Text _getDialogText(BuildContext context, EnrollmentExists enrollmentExists) {
  ...
  text = enrollmentExists.emailCount > 1 || enrollmentExists.phoneCount > 1
      ? text.replaceFirst("[MEMBERCOUNT]", FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string4"))
      : text.replaceFirst("[MEMBERCOUNT]", FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string5"));
  text = enrollmentExists.emailExists
      ? text.replaceFirst("[MEMBEREMAIL]", FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string6"))
      : text.replaceFirst("[MEMBEREMAIL]", "");
  text = enrollmentExists.emailExists && enrollmentExists.phoneExists
      ? text.replaceFirst("[MEMEBERAND]", FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string7"))
      : text.replaceFirst("[MEMEBERAND]", "");
  text = enrollmentExists.phoneExists
      ? text.replaceFirst("[MEMBERPHONE]", FlutterI18n.translate(context, "enrollment.enrollmentFormFunctions.string8"))
