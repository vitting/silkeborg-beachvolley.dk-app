...
class TournamentFirestore {
  ...
  static Future<QuerySnapshot> getTournaments() {
    return firestoreInstance.collection(_collectionName).orderBy("startDate").getDocuments();
  }

  static Future<void> saveTournament(TournamentData tournament) {
    return firestoreInstance.collection(_collectionName).document(tournament.id).setData(tournament.toMap());
  }

  static Future<void> deleteTournament(String tournamentId) {
    return firestoreInstance.collection(_collectionName).document(tournamentId).delete();
  }
